      ...
      <q-form @submit="onSubmit">
        <q-card-actions class="q-pa-none q-pl-xs">
          <q-toolbar class="bg-redx text-black">
            <q-toolbar-title> KONFIRMASI PERGANTIAN SHIFT </q-toolbar-title>
        ...
        >
          ...
          <div v-if="!isCheckDone" class="row q-col-gutter-md">
            <div class="col-6">
              <!-- <div v-if="getPemberi?.id" class="col-6 q-mb-md">
                <q-list bordered>
                  <q-item>
                    <q-item-section avatar>
                      <q-avatar>
                        <img
                          :src="getPemberi?.foto ? getPemberi?.foto : $defaultImage1"
                        />
                      </q-avatar>
                    </q-item-section>

                    <q-item-section>
                      <q-item-label caption>Kasir PEMBERI</q-item-label>
                      <q-item-label>{{
                        getPemberi?.nama ? getPemberi?.nama : "PEMBERI KOSONG"
                      }}</q-item-label>
                    </q-item-section>

                    <q-item-section side>
                      <q-item-label caption>SHIFT</q-item-label>
                      <q-item-label>{{ getPemberi?.shift }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </q-list>
              </div> -->
              ...
              <q-select
                ...
                :error="
                  ...
                  rotasi_cashier_pemberi == rotasi_cashier_penerima
                "
                :emit-value="false"
                option-label="nama"
            ...
            <div class="col-6">
              <!-- <div v-if="getPenerima?.id" class="col-6 q-mb-md">
                <q-list bordered>
                  <q-item>
                    <q-item-section avatar>
                      <q-avatar>
                        <img
                          :src="getPenerima?.foto ? getPenerima?.foto : $defaultImage1"
                        />
                      </q-avatar>
                    </q-item-section>
                    ...
                    <q-item-section>
                      <q-item-label caption>Kasir PEMBERI</q-item-label>
                      <q-item-label>{{
                        getPenerima?.nama ? getPenerima?.nama : "PEMBERI KOSONG"
                      }}</q-item-label>
                    </q-item-section>

                    <q-item-section side>
                      <q-item-label caption>SHIFT</q-item-label>
                      <q-item-label>{{ getPenerima?.shift }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </q-list>
              </div> -->
              ...
              <q-select
                ...
                :error="
                  ...
                  rotasi_cashier_penerima == rotasi_cashier_pemberi
                "
                outlined
  ...
  computed: {
    ...
    getPemberi() {
      ...
      this.ABSENSI_struks.forEach((el) => {
        if (el?.cashier_id == this.rotasi_cashier_pemberi?.id) temp = el;
      });
      return temp;
    },
    getPenerima() {
      let temp = null;
      this.ABSENSI_struks.forEach((el) => {
        if (el?.cashier_id == this.rotasi_cashier_penerima?.id) temp = el;
      });
      return temp;
    },
  },
    ...
    async onSubmit() {
      if (this.isCheckDone) {
        return this.$q.notify({
          message: "Peringatan",
          caption: "Maksimal PERGANTIAN SHIFT 1 kali sehari",
          icon: "warning",
          color: "negative",
          position: "top",
        });
