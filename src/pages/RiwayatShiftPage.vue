<template>
  <q-page class="q-pa-sm row">
    <DialogTableRiwayatShift ref="dialog_table_riwayat_shift"></DialogTableRiwayatShift>
    <q-date
      subtitle="LAPORAN REKAP SHIFT"
      class="full-width"
      color=" bg-sw"
      v-model="date"
      :locale="myLocale"
...
import DialogTableRiwayatShift from "src/components/DialogTableRiwayatShift.vue";
...
export default {
  ...
  watch: {
    date(val) {
      ...
      if (!this.date) return;
      this.$refs.dialog_table_riwayat_shift.onOpen(this.date);
    },
  },
  data() {
    return {
