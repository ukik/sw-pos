<template>
  <q-page class="flex flex-center row q-py-lg">
    <div class="col-12 text-center">
      <img class="q-pb-lg" alt="Quasar logo" src="images/logo.jpg" style="width: 200px; height: auto" />
    </div>
    ...
    <div class="col-xl-6 col-lg-6 col-md-7 col-sm-10 col-xs-12">
      <q-list separator="" class="col-12" bordered>
        ...
        <q-item :to="{ name: 'pengaturan_kasir' }" clickable v-ripple>
          ...
          <q-item-section side>
            <q-icon size="xl" :name="getKasirLabelColor?.icon" :color="getKasirLabelColor?.color" />
          </q-item-section>
        </q-item>
        ...
        <q-item :to="{ name: 'pengaturan_kurir' }" clickable v-ripple>
          ...
          <q-item-section side>
            <q-icon size="xl" :name="getKurirLabelColor?.icon" :color="getKurirLabelColor?.color" />
          </q-item-section>
        </q-item>
        ...
        <q-item :to="{ name: 'absensi' }" clickable v-ripple>
          ...
          <q-item-section side>
            <q-icon size="xl" :name="getPiketSudahAbsensiColor?.icon" :color="getPiketSudahAbsensiColor?.color" />
          </q-item-section>
        </q-item>
        ...
        <q-item @click="onDialogGantiKasir" clickable v-ripple>
          ...
          <q-item-section side>
            <q-icon size="xl" :name="getPiketLabelColor?.icon" :color="getPiketLabelColor?.color" />
          </q-item-section>
        </q-item>
      </q-list>
...
export default {
  computed: {
    ...
    getPiketLabel() {
      return this.cashier?.id ? `Bertugas sekarang ${this.cashier?.nama}` : `Tidak boleh kosong`
    },
    getKasirLabel() {
      return this.list_cashiers?.length > 0 ? `Terdapat ${this.list_cashiers?.length} orang kasir` : `Tidak boleh kosong`
    },
    getKurirLabel() {
      return this.list_courirs?.length > 0 ? `Terdapat ${this.list_courirs?.length} orang kurir` : `Tidak boleh kosong`
    },
    getPiketLabelColor() {
